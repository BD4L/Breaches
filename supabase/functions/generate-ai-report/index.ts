...
// -------------------
// Helper: Build concise, focused prompt for Kimi-K2
// -------------------
function buildSimplifiedKimiPrompt(company: string): string {
  return `Search the web (news outlets, government breach portals, regulatory filings, PDFs, and reputable blogs) for information about the data breach at **${company}**.

Return a concise markdown report that includes:
1. **Company confirmation** – verify the company is exactly "${company}"; if not, STOP.
2. **Breach timeline** – when the attack occurred, when discovered, when publicly disclosed.
3. **Victim impact** – exact number of individuals affected (cite source) and what data types were leaked.
4. **Notifications** – whether the company has emailed / notified victims yet (cite).
5. **Target demographic** – describe the company’s customer demographics (age, income, geography) to inform marketing outreach for affected users.
6. **Estimated damages per consumer** if available (cite).

Requirements:
• Inline markdown citations for every fact, formatted like [Source](URL).
• Use bullet lists or short paragraphs – no unnecessary fluff.
• Only include information backed by reputable sources (AG sites, SEC, HHS, major news, PDF notices, court dockets).`;
}
...
`;
  try {
    // ----- Prefer Kimi-K2 if we have an OpenRouter key -----
    if (OPENROUTER_KEY) {
      console.log('🤖 Initiating AI research with Kimi-K2 (OpenRouter)…');
      const kimiText = await runWithKimi([
        {
          role: 'system',
          content: 'You are an expert cybersecurity and market-intelligence analyst. Provide well-structured answers in markdown with inline citations.'
        },
        {
          role: 'user',
          content: buildSimplifiedKimiPrompt(organization)
        }
      ]);
...
async function conductDemographicResearch(organization, geminiApiKey) {
  const genAI = new GoogleGenerativeAI(geminiApiKey);
  const model = genAI.getGenerativeModel({
    model: "gemini-2.5-flash",
    tools: [
...
`;
  try {
    if (OPENROUTER_KEY) {
      console.log('🤖 Running demographic research with Kimi-K2…');
      const kimiDemo = await runWithKimi([
        { role: 'system', content: 'You are a market analyst. Provide customer demographic insights with markdown citations.' },
        { role: 'user', content: demographicPrompt }
      ]);
      return { research_text: kimiDemo, demographic_sources: [] };
    }
...
async function generateComprehensiveReport(breach, researchData, demographicData, geminiApiKey) {
  const genAI = new GoogleGenerativeAI(geminiApiKey);
  const model = genAI.getGenerativeModel({
    model: "gemini-2.5-flash"
  });
...
function generateFallbackReport(breach, researchData, demographicData) {
